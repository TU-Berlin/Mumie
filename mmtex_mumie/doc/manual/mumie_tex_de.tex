% $Id: mumie_tex_de.tex,v 1.68 2010/02/04 09:34:59 rassy Exp $

\documentclass[12pt]{article}

\RequirePackage{mumie_tex}
\usepackage{rotating}
...
%\else
%  \includegraphics[width=2cm,angle=0]{pics/mumie_small.pdf}
%\fi
%\vspace*{3cm}

%\textsf{CVS-Id:} \verb'$Id: mumie_tex_de.tex,v 1.68 2010/02/04 09:34:59 rassy Exp $'

\begin{tabular}{ll}
\textsf{Maintainer:}& Tilman Rassy  \\
\textsf{Datum:}&
   \verb+$Date: 2010/02/04 09:34:59 $+\\ % Wird vom cvs eingetragen
\textsf{CVS-Version:}&
   \verb+$Revision: 1.68 $+\\            % Wird vom cvs eingetragen
\textsf{CVS-Source:}&
   \verb+$Source: /net/mumie/cvs/mmtex_mumie/doc/manual/mumie_tex_de.tex,v $+\\  % Wird vom cvs eingetragen
\end{tabular}
...
\ref{section:templatestheorem}). Damit sind für \verb+lemma+
die gleichen Umgebungen definiert wie für \verb+theorem+.
...
Die \verb|background|-Umgebung ist optional und soll die Möglichkeit bieten, die tragenden Ideen eines Algorithmus in knapper Form zu präsentieren.
...
\subsubsection{Beispiel für eine einfache Liste}\label{subsubsection:list-example-plain}
...
\subsubsection{Beispiel für eine nummerierte Liste}\label{subsubsection:list-example-enum}
...
Punkten der Liste vorangestellt werden soll. Folgende Symbole sind verfügbar:
\end{description}
...
Sowohl einfache als auch nummerierte Listen können in jedem
...
\subsubsection{Kopf, Hauptteil, Fuß}

Innerhalb der Tabellenumgebung stehen folgende Befehle zur Verfügung:
...
Sie leiten die jeweiligen Abschnitte ein. Für den Hauptteil können
zusätzlich optional Parameter für die Ausrichtung der Zellinhalte
jeder Spalte angeben werden. Dazu siehe: \ref{subsubsection:table-param}.
Für jeden Abschnitt der Tabelle sind mehrere Zeilen erlaubt.
...
der zu verbindenen Zeilen bzw. Spalten. Optional kann auch hier für jeden
Verbund eine eigene Klasse angegeben werden. Dazu siehe: \ref{subsubsection:table-param}.

Spezifiziert benutzerdefinierte Klassen für Tabellen. Die Definition erfolgt
ausserhalb des \texttt{content}-Blocks und wird, wie im Beispiel gezeigt,
bei der Öffnung einer Tabellenumgebung referenziert.
